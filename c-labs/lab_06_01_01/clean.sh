...
# Ошибка того, что нет файла в директории
DIR_NOT_EXISTS=2
# КОд возврата успешной очистки директорий проекта
SUCCESS=0
...
RIGHT_FIRST_LEVEL_FILES_NAMES=("main.c" "build_release.sh" "build_debug.sh" "clean.sh")
...
for file in *; do
    ...
    for right_name in "${RIGHT_FIRST_LEVEL_FILES_NAMES[@]}"; do
        if [ "$right_name" == "$file" ] || [[ "$file" =~ .+\.c ]] || [[ "$file" =~ .+\.h ]]; then
	    counter="$((counter+1))"
...
for file in ./func_tests/scripts/*; do
    ...
    if [ -f "$file" ] && [ "$filename" != "func_tests.sh" ] && [ "$filename" != "neg_case.sh" ] && [ "$filename" != "pos_case.sh" ] && [ "$filename" != "comparator.sh" ]; then
        rm -f "$file"
    fi
...
for file in ./func_tests/data/*; do
    ...
    if ! echo "$filename" | grep -Eq "^(pos|neg)_[0-9]{2}_(in|out|args)\.txt"; then
        rm -f "$file"
	continue
    fi
done
